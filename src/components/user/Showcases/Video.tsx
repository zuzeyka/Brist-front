        ...
        return (
            <div className="bg-card2 rounded-2xl w-full p-4">
                <div className="flex flex-col space-y-4">
                    <h2 className="text-heading-1 font-bold font-manrope">Галерея відео</h2>
                    ...
                    {videosUrl.length == 2 ? (<video className="w-full h-96 object-cover rounded-2xl" height="100" controls src={videosUrl[1]}></video>) : (
                        <div className="flex space-x-2">
                            ...
                            {videosUrl.length > 3 ? (<div className="h-32 w-1/3 object-cover rounded-2xl bg-gray-400 flex items-center justify-center text-typographySecondary text-sign-1">+{videosUrl.length - 3}</div>) : null}
                        </div>
                    )}
                </div>
            </div>
