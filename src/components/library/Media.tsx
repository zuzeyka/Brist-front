    ...
    const renderMediaContent = () => {
        if (props.postMediaUrl) {
            if (props.postMediaUrl.match(/\.(mp4|webm)$/i)) {
                return <video className='w-auto h-96 rounded-md my-4 object-cover' src={props.postMediaUrl} controls></video>;
            } else {
                return <img className='w-auto h-full rounded-md my-4 object-cover' src={props.postMediaUrl} alt="Media content"></img>;
            }
        }
        return null;
    };
